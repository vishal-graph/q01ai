...
router.post('/questionnaires/:id/messages', async (req, res) => {
  ...
  if (!nextId) {
    ...
    try {
      summary = await generateProjectSummary(doc.service, doc.parameters as Record<string, string>);
      console.log('[debug] Generated project summary:', JSON.stringify(summary));
    } catch (err) {
    ...
    return res.json({ id: doc.id, status: 'completed', parameters: doc.parameters, summary });
  }
